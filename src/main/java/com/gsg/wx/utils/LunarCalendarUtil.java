...
public class LunarCalendarUtil {
     ...
     *
     * @param date 农历
     *             isLeapMonth   该月是否是闰月(该参数可以根据本类中leapMonth()方法，先判断一下要查询的年份是否有闰月，并且闰的几月)
     * @return 返回农历日期对应的公历日期，year0, month1, day2.
     */
    public static final String lunarToSolar(String date) {
        boolean isLeapMonth = false;
        String[] array = date.split("-");
        if (leapMonth(Integer.parseInt(array[0])) > 0) {
            isLeapMonth = true;
        }

        int year = Integer.parseInt(array[0]);
        int month = Integer.parseInt(array[1]);
        int monthDay = Integer.parseInt(array[2]);
        ...
        for (i = 1; i < 13; i++) {
            ...
            if (iPos >= dayOffset) {
                ...
                if (dayOffset > iPos) {
                    monthDay = dayOffset - iPos;
                } else if (dayOffset == iPos) {
                    if (year % 4 == 0 && i == 2) {
                        monthDay = DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1] + 1;
                    } else {
                        monthDay = DAYS_BEFORE_MONTH[i] - DAYS_BEFORE_MONTH[i - 1];
                    }
                } else {
                    monthDay = dayOffset;
                }
        ...
        String rDate;
        if (month < 10 || monthDay < 10) {
            if (month < 10 && monthDay < 10) {
                rDate = year + "-0" + month + "-0" + monthDay;
            } else if (month < 10) {
                rDate = year + "-0" + month + "-" + monthDay;
            } else {
                rDate = year + "-" + month + "-0" + monthDay;
            }
        } else {
            rDate = year + "-" + month + "-" + monthDay;
        }
    ...
    public static void main(String[] args) {
        //计算录入年份是否有闰月
//        System.out.println(leapMonth(2022));
//        //传回农历 year年month月的总天数，总共有13个月包括闰月
...
//        System.out.println(Arrays.toString(solarToLunar(2020, 3, 2)));
        //阴历转阳历
        System.out.println(lunarToSolar("2021-01-24"));
    }
}